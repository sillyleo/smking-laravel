...
canonical. Those tags rendered inside `<body>` (wherever the component sat),
where HTML ignores them — the host page's `<head>` defaults silently won.
Blade has no React-19-style metadata hoisting, so the markup was dead.

CMS SEO meta **and a new JSON-LD `@graph`** (BlogPosting / Organization /
WebSite / BreadcrumbList / FAQPage, `@id`-linked) are now generated by the
SaaS structured-data engine and served via `/api/v1/public/aeo`. The
`InjectAeo` middleware injects them into `<head>` — where they're honored —
on every CMS page request, the same path it already uses for AEO content.

**Upgrade**: `composer update smking/laravel` (bump your constraint to
`^0.20`). No customer code change. **Verify** the `InjectAeo` middleware
covers your CMS routes — `curl -I https://yoursite/your-cms-page` should
return an `X-Smking-Status` header. Without the middleware on that route,
CMS pages get no SEO meta (the old in-body tags were non-functional anyway).
...
- **Mode A (`'css'`, default)** — `cms.blade.php` emits `<style data-smking="cms-styles">` once per render carrying the canonical SDK stylesheet (regenerated by `pnpm sync:cms-styles` from `packages/shared/src/styles/cms-styles.css`). Class hooks `.smk-cms` / `.smk-block--*` / `.smk-hero` / `.smk-nav` / `.smk-carousel`. Customer themes via CSS variables on `:root` (`--smk-accent`, `--smk-font-serif`, etc).
- **Mode B (`'tailwind-prose'`)** — set `SMKING_CMS_THEME=tailwind-prose` in `.env` or `config('smking.cms.theme_mode')`. `cms.blade.php` wraps the article in `<article class="prose dark:prose-invert">` and delegates rendering to a new `block-prose.blade.php` partial that emits Tailwind utility classes (`not-prose` resets around hero/nav/carousel/search). Requires `@tailwindcss/typography` + the SDK's utility classes in the Tailwind safelist (`@soloworks/smking-wizard` 0.4+ handles both).
- **`config/smking.php` → `cms.theme_mode`** — new key (`env('SMKING_CMS_THEME', 'css')`). Customers upgrading from v0.14.x can leave it unset for legacy behavior.
- **`block-prose.blade.php`** — Mode B single-block dispatcher mirror of `block.blade.php`.

### Changed

- `cms-styles.blade.php` is now AUTO-GENERATED from `packages/shared/src/styles/cms-styles.css`. Edits go upstream — re-running `pnpm sync:cms-styles` overwrites local changes. The CSS payload is identical byte-for-byte to the version emitted by `@soloworks/smking-next` 0.14.0.

### Tested

- Phpunit suite 171/171 green on testbench Laravel 12.
- Mode toggle smoke test against the boss customer site (`sleepytofu.com.test`): Mode A renders `.smk-cms` + canonical stylesheet; flipping `SMKING_CMS_THEME=tailwind-prose` immediately switches markup to `prose`-wrapped blocks (no SDK code change needed).

### Customer

Composer caret rule: `"smking/laravel": "^0.14"` resolves only `>=0.14 <0.15`. Bump constraint to `"^0.15"` and run `composer update smking/laravel` (or just run the wizard — it patches `composer.json` for you).
...
Mode B opt-in requires `@tailwindcss/typography` + safelist on the customer side; let the wizard handle it (`npx @soloworks/smking-wizard@latest`) or follow the install prompt section in the package README.
